...
const useCalculateProfit = () => {
    ...
    const calculateProfit = (allCoins:ICoinsResponse, favouriteCoins: ILocalStorageCoinKey[]) => {
        ...
        favouriteCoins.forEach((favouriteCoin) => {
            ...
            portfolioPriceCurrent += Number(currentPrice(allCoins, favouriteCoin.coinId))*favouriteCoin.quantity 
        })

        const profit = portfolioPriceCurrent-portfolioPrice
        const profitPercent = ((portfolioPriceCurrent-portfolioPrice)/portfolioPrice)*100
